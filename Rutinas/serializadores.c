...
char* serializar_mensaje_ES(t_iESdeCPU* datosES){
	int size=0, offset =0;
	size = (sizeof(int32_t)*2) + datosES->tamanioID;
...
t_iESdeCPU * deserializar_mensaje_ES(char * payload, uint32_t longitud){
        t_iESdeCPU* datosES;
		int size=0, offset=0;
		datosES = malloc(longitud);
		size = sizeof(int32_t);
...
t_iVARCOM * deserializar_datos_variable(char * mensaje, uint32_t longitud){
    ...
    t_iVARCOM * datos_variables = malloc(longitud);
    ...
    offset += tmp_size;
    tmp_size = sizeof(int32_t); //pasamos mensajes cadena con "\0"
